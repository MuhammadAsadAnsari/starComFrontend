...
const DataTable = ({
  ...
  return (
    <table className="min-w-full border border-gray-300 bg-white text-gray-800 rounded-lg shadow-md overflow-hidden">
      <thead>
        <tr className="bg-gray-100 text-gray-800">
          {headers.map((header, index) => (
            <th key={index} className={`py-3 px-4 ${styling && styling}`}>
              {header}
            </th>
          ))}
        </tr>
        ...
        {data.length > 0 ? (
          data.map((item, index) =>        {
            ...
            return (
              ...
              >
                <td className={`${styling && "p-3"} py-3 text-center`}>
                  {item?.client?.name ||
                    item?.name ||
                    item?.timing ||
                    item.channel}
